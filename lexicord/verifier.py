...
class JapaneseTermVerifier(object):
    def __init__(self, mecab_tagger):
        ...
        self._homonym = defaultdict(lambda: nltk.ConditionalFreqDist())
    ...
    def _tokenize(self, sentence):
        ...
        while node and node.surface:
            tokens.append((node.surface.decode('utf-8'),
                           (node.feature.decode('utf-8').split(u',') + [u'', u'', u'', u''])[:8],
                           node.cost))
            node = node.next
        return tokens
    ...
    def feed_sent(self, sentence):
        ...
        for (surface, feature, _) in tokens:
            ...
            self._examples2[surface+u','+feature[0]] = len(self._sentences) - 1
            # katakana
            self._katakana.train_by_token(surface)
            # homonym
            if feature[7]:
                self._homonym[feature[7]][feature[0]][surface] += 1
    ...
    def print_katakana_inconsistency(self):
        ...
        for kl in self._katakana.inconsistent():
            print(u'----------')
            for (w, f) in kl:
    ...
    def print_homonym(self):
        ...
        for yomi in sorted(self._homonym):
            for pos in sorted(self._homonym[yomi]):
                if len(self._homonym[yomi][pos]) > 1:
                    print(u'[{}] ({})'.format(yomi, pos))
                    for (w, f) in sorted(self._homonym[yomi][pos].items(), reverse=True, key=lambda x: x[1]):
                        print(u'  {} ({})'.format(w, f))
                        print(u'    {0}'.format(self._sentences[self._examples2[w+u','+pos]]))
...
if __name__ == '__main__':
    ...
    bsl = BiSegList()
    _ = bsl.read(format='tmx', name=myenv.tmxV, encoding='utf-8', verbose=True, metatextparser=htmldecode)
    ...
    # bsl.prune(bsl.research(u'abcdefghijklmnopqrstuvwxyz', where='target'))
    ...
    #     verifier.feed_sent(t)
    # for t in sample1:
    #     tokens = verifier._tokenize(t)
    #     print(u' '.join(s for (s, f, c) in tokens))
    #     f = verifier._katakana.notational((s for (s, f, c) in tokens), use_most_frequent=True)
    #     if f:
    #         print(u', '.join(f))
